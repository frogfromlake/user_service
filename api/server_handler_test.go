...
func TestStartServer(t *testing.T) {
	...
	testCases := []struct {
		...
		pingError   bool
		invalidPort bool
		buildStubs  func(store *mock_db.MockStore, pingError bool)
	}{
		{
			...
			buildStubs: func(store *mock_db.MockStore, pingError bool) {
				if !pingError {
					store.EXPECT().Ping(gomock.Any(), gomock.Any()).Times(1).Return(nil)
				} else {
					store.EXPECT().Ping(gomock.Any(), gomock.Any()).Times(1).Return(errors.New("database not ready"))
				}
			},
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.buildStubs(mockStore, tc.pingError)

			server := NewServer(mockStore)

			// Get a random port or an invalid port
			port, err := getRandomPort()
			if tc.invalidPort {
				port = -1
			}
			if err != nil && !tc.invalidPort || port <= 0 && !tc.invalidPort {
...
// func convertUtilAccountTypesToDbAccountTypes(utilAccountTypes []util.AccountType) []db.StreamfairAccountType {
// 	dbAccountTypes := make([]db.StreamfairAccountType, len(utilAccountTypes))
// 	for i, utilAccountType := range utilAccountTypes {
// 		dbAccountTypes[i] = db.StreamfairAccountType{
...
// 	testCases := []struct {
// 		name         string
// 		accountTypes []db.StreamfairAccountType
...
// 				store.EXPECT().ListAccountTypes(gomock.Any(), gomock.Any()).Times(1).Return([]db.StreamfairAccountType{}, nil)
...
// 						IsWriter:    accountType.IsWriter,
// 						IsLabel:     accountType.IsLabel,
// 					}).Times(1).Return(db.StreamfairAccountType{}, errors.New("another error"))
// 				}
// 			},
